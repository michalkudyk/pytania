...
class QuestionWindow:
    def __init__(self, master, questions):
        ...
        self.engine.setProperty('rate', 200)  # Zwiększenie prędkości odczytu
    ...
    def display_text(self):
        if self.current_question_index < len(self.questions):
            ...
            self.master.after(1000, self.read_question_aloud)  # Czytanie pytania po 1 sekundzie

    def read_question_aloud(self):
        if self.current_question_index < len(self.questions):
            current_question = self.questions[self.current_question_index]
            self.engine.say(current_question)  # Odczytanie pytania głośno
            self.engine.runAndWait()
            self.current_question_index += 1
            self.master.after(10000, self.display_text)  # Wyświetlenie nowego tekstu po 20 sekundach
